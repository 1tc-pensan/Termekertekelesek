...
## 📚 XVIII. TOVÁBBI FEJLESZTÉSI LEHETŐSÉGEK
...
3. **Pagination Customization**
   ```php
   Products::paginate(10); // 10 item/oldal
   ```

4. **File Upload (termék képek)**
   ```bash
...
2. **Search & Filtering**
   ```php
   Products::where('name', 'like', '%laptop%')->get();
   ```
...
2. **WebSocket (Real-time reviews)**
   ```bash
   composer require beyondcode/laravel-websockets
   ```

3. **Queue Jobs (Email notifications)**
   ```bash
   php artisan queue:table
   php artisan make:job SendReviewNotification
   ```
